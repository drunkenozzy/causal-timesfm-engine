...
def run_housing_analysis(property_price=420000.0, postcode="UB3 4GL"):
    ...
    falsify = f"Thesis falsified if local mortgage rates exceed 6.5% or service charges rise above £4,500/yr."
    summary = reconciler.generate_plain_english_summary(
        asset_name=f"2-Bed Flat, {postcode}",
...
def currency_symbol(ticker):
    return "£" if "GBP" in ticker or "UB3" in ticker else "$"
...
def main():
    ...
    parser.add_argument("--postcode", default="UB3 4GL")
