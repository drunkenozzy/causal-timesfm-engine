...
def apply_fx_mechanism(df_hist, m1_p50):
    ...
    df['d_DXY'] = df['DXY'].diff()
    df['d_TNX'] = df['TNX'].diff()
    df['d_Oil'] = df['Brent'].diff()
    
    # Target: Next-day USD/TRY return
    df['Next_Ret'] = df['TRY'].pct_change().shift(-1)
    
    # Drop NAs
    df_train = df.dropna(subset=['d_DXY', 'd_TNX', 'd_Oil', 'Next_Ret']).copy()
    ...
    X = df_train[['d_DXY', 'd_TNX', 'd_Oil']].values
    ...
    # Current features
    current_d_DXY = df['d_DXY'].iloc[-1]
    current_d_TNX = df['d_TNX'].iloc[-1]
    current_d_Oil = df['d_Oil'].iloc[-1]
    
    if np.isnan(current_d_DXY) or np.isnan(current_d_TNX) or np.isnan(current_d_Oil):
        return m1_p50, 0.0
        
    # Expected extra return driven by the macro variables
    expected_ret_adj = model.predict([[current_d_DXY, current_d_TNX, current_d_Oil]])[0]
