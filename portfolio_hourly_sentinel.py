...
class PortfolioHourlySentinel:
    def __init__(self, portfolio_value=10000.0, crypto_pct=30.0, interval_seconds=3600, output_json_path="dashboard_risk_status.json", webhook_url=None):
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Portfolio Hourly Risk Sentinel")
    parser.add_argument("--portfolio-value", type=float, default=10000.0, help="Total portfolio value in USD (default: 10000)")
    parser.add_argument("--crypto-pct", type=float, default=30.0, help="Current crypto allocation percentage (default: 30)")
    parser.add_argument("--interval", type=int, default=3600, help="Check interval in seconds (default: 3600)")
    parser.add_argument("--continuous", action="store_true", help="Run continuously as a daemon")
    args = parser.parse_args()

    sentinel = PortfolioHourlySentinel(
        portfolio_value=args.portfolio_value,
        crypto_pct=args.crypto_pct,
